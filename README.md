...
- Our group has aimed for the optional sidegoal, to also use a MySQL database, but as a third container, to handle both web and SQL queries.
...
- This script asks for a dadjokes.json from the webserver using curl, prints a random joke in the terminal, and saves that random joke in the MySQL database. This loops every 10 seconds.
- When the containers are built, the script will before starting the loop, wait and make sure web and MySQL containers are up and responsive.
- The script continuesly makes sure that both the web and MySQL container are responsive by checking for healthy HTTP codes.
